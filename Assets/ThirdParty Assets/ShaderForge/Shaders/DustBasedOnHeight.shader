...
///////// Gloss:
                float node_4457 = 0.0;
                float gloss = 1.0 - node_4457; // Convert roughness to gloss
                float perceptualRoughness = node_4457;
...
////// Specular:
                ...
                float3 specularColor = node_4457;
                ...
                float3 node_3827 = lerp(_DustTexture_var.rgb,_RockBase.rgb,(_node_2582_var.r*_dustnoiseopacity));
                float node_3972 = 0.0;
                float3 diffuseColor = lerp(_RockBase.rgb,lerp(node_3827,_RockBase.rgb,_overallopacity),saturate(pow((max(0,dot(float3(node_3972,_UpNode,node_3972),((i.posWorld.g-objPos.g)*_worldpos)))*_Level),_contrast))); // Need this for specular when using metallic
